...
set _DeploySetParametersFile=%RootPath%Orchard.Web.SetParameters.xml
)
...
@rem ---------------------------------------------------------------------------------
if not exist "%RootPath%Orchard.Web.zip" (
echo "%RootPath%Orchard.Web.zip" does not exist. 
...
set _MSDeployCommandline="%MSDeployPath%msdeploy.exe" -source:package='%RootPath%Orchard.Web.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -skip:objectname='dirPath',absolutepath='obj\\Release\\Package\\PackageTmp\\App_Data$' -skip:objectname='dirPath',absolutepath='CertifiedOverheadCrane\\App_Data$'
) else (
...
:usage
echo =========================================================
if not exist "%RootPath%Orchard.Web.deploy-readme.txt" (
...
start notepad "%RootPath%Orchard.Web.deploy-readme.txt"
)
echo =========================================================
goto :eof
