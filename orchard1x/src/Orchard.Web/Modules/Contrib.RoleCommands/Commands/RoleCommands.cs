...
namespace Contrib.RoleCommands.Commands
{
    public class RoleCommands : DefaultOrchardCommandHandler
    {
        ...
        public RoleCommands(IRoleService roleService, IRepository<UserPartRecord> userRepository, IRepository<UserRolesPartRecord> userRolesRepository)
        {
        ...
        private void PrintRoleRecord(RoleRecord roleRecord, int initialIndent = 0)
        {
            ...
            if (IncludeUsers)
            {
                ...
                {
                    var userPartRecord = _userRepository.Get(userRolesPartRecord.UserId);

                    Context.Output.Write(new string(' ', secondIndent));
                    Context.Output.Write("UserName:".PadRight(15));
                    Context.Output.WriteLine(userPartRecord.UserName);

                    Context.Output.Write(new string(' ', secondIndent));
                    Context.Output.Write("Email:".PadRight(15));
                    Context.Output.WriteLine(userPartRecord.Email);
                    Context.Output.WriteLine();
                }
            }
        }
        ...
        {
            var userPartRecord = _userRepository.Get(record => record.UserName == username);
            ...
            foreach (var rolesPartRecord in _userRolesRepository.Fetch(record => record.UserId == userPartRecord.Id, order => order.Asc(record => record.Role.Name)))
            {
                Context.Output.Write(new string(' ', 2));
                Context.Output.WriteLine(rolesPartRecord.Role.Name);
            }
        }
        ...
        public void UserAddRole(string username, string role)
        {
            ...
            Context.Output.WriteLine(T("Adding role {0} to user {1}", roleRecord.Name, userPartRecord.UserName));
        ...
        public void UserRemoveRole(string username, string role)
        {
            ...
            Context.Output.WriteLine(T("Removing role {0} from user {1}", roleRecord.Name, userPartRecord.UserName));
